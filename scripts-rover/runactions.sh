...
if [[ -z ${env} ]]; then
  ...
  echo "expecting: ./runactions.sh <environment name>"
  echo ""
  exit 1
fi
...
read -p 'Do you want to proceed with the deployment via a pipeline (yes/no): ' proceed
...
if [ "${proceed,,}" = "yes" ];
then
  read -p 'Please provide a commit message for this deployment: ' commit_message
  
  mkdir -p .actions
  echo "/tf/rover/gorover.sh ${env} plan" > .actions/cicdrun.sh
  echo "/tf/rover/gorover.sh ${env} apply -auto-approve" >> .actions/cicdrun.sh
  chmod +x .actions/cicdrun.sh
  git add .
  git commit -m "${commit_message}: runactions ${env} ${command} in ${blueprint} on ${date}"
